...
void tcp_get_ip_address(rdpTcp * tcp)
{
	...
	{
		ip = (unsigned char*) (&sockaddr.sin_addr);
...
boolean tcp_connect(rdpTcp* tcp, const char* hostname, unsigned short port)
{
	...
	setsockopt(tcp->sockfd, IPPROTO_TCP, TCP_NODELAY, (char *)&option_value, option_len);

	/* receive buffer must be a least 32 K */
	if (getsockopt(tcp->sockfd, SOL_SOCKET, SO_RCVBUF, (char*)&option_value, &option_len) == 0)
	{
		if (option_value < (1024 * 32))
		{
			option_value = 1024 * 32;
			option_len = sizeof(option_value);
			setsockopt(tcp->sockfd, SOL_SOCKET, SO_RCVBUF, (char*)&option_value, option_len);
		}
	}
